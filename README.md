...
### Execução
- Apenas compile a main.cpp, os outputs serão escritos na pasta */Raytracing/outputs*
````
...
## Projeto no webGL
### Descrição
Configuramos um ambiente WebGL para renderizar uma esfera e uma pirâmide em 3D, usamos a biblioteca gl-matrix, operações de matriz de alta performance para programação gráfica.
...
- Vertex Shader E Fragment shader
	- São definidos da mesma maneira que a esfera

- InitDemo()
	- Aqui, temos um código bastante similar, mas com algumas diferenças:
	- Os dados dos vértices da pirâmide são definidos diretamente em um array *pyramidVertices*
	- Usa um buffer de vértices e um buffer de índices específicos para a pirâmide.
	- Define a cor dos vértices diretamente no array pyramidVertices.
	- Não utiliza funções externas para a definição de geometrias, ou seja, o código da pirâmide está hardcoded na função.
````
